  ...
  porta_serial
    ...
    .then(entradas_seriais => {
      ...
      var entradas_seriais_arduino = entradas_seriais.filter(entrada_serial => {
        return (
          entrada_serial.vendorId == 2341 && entrada_serial.productId == 43
        );
      });

      if (entradas_seriais_arduino.length != 1) {
        throw new Error(
          "Nenhum Arduino está conectado ou porta USB sem comunicação ou mais de um Arduino conectado"
        );
      }

      console.log(
        "Arduino conectado na COM %s",
        entradas_seriais_arduino[0].comName
      );
...
function registrar_leitura(Entrada) {
  ...
  banco
    ...
    .then(pool => {
      return pool.request()
        .query(`insert into eventos (evento) values ('${Entrada}');
